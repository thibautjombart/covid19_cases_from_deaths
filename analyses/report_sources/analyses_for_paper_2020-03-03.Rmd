...
if(Sys.info()["sysname"] == "Windows"){
  sim_deaths <- function(death_dates, params = NULL, duration = 0) {
...
fr_first_death_dates <- as.Date('2020-02-26')

fr_max_date <- fr_first_death_dates + 7

sims_fr_cases <- sim_deaths(fr_death_dates, params, duration = 8)


fr_max_date <- max(fr_death_dates)
  
## get table
results_fr_cases <- get_summaries(sims_fr_cases, fr_max_date, params)
...
results_fr_cases$deaths <- length(fr_first_death_dates)
...
it_first_death_dates <- it_death_dates[it_death_dates == min(it_death_dates)]

it_max_date <- max(it_first_death_dates) + 10

sims_it_cases <- sim_deaths(it_first_death_dates, params, duration = 11)

results_it_cases <- get_summaries(sims_it_cases, it_max_date, params)

it_cases_total <- df %>% filter(country == "Italy",
                                   date <= it_max_date) 

it_reported_cases <- sum(it_cases_total$new_case)

results_it_cases$reported <- it_reported_cases
results_it_cases$deaths <- length(it_first_death_dates)
...
# Get the deaths of the first dates
jp_first_death_dates <- jp_death_dates[jp_death_dates == min(jp_death_dates)]


sims_jp_cases <- sim_deaths(jp_first_death_dates, params, duration = 15)

jp_max_date <- max(jp_first_death_dates) + 14

results_jp_cases <- get_summaries(sims_jp_cases, jp_max_date, params)
...
results_jp_cases$deaths <- length(jp_first_death_dates)
...
sk_first_death_dates <- sk_death_dates[sk_death_dates == min(sk_death_dates)]


sims_sk_cases <- sim_deaths(sk_first_death_dates, params, duration = 15)

sk_max_date <- max(sk_first_death_dates) + 14

results_sk_cases <- get_summaries(sims_sk_cases, sk_max_date, params)
...
results_sk_cases$deaths <- length(sk_first_death_dates)
...
ch_first_death_dates <- ch_death_dates[ch_death_dates == min(ch_death_dates)]

ch_first_death_dates + 14


sims_ch_cases <- sim_deaths(ch_first_death_dates, params, duration = 15)

ch_max_date <- max(ch_first_death_dates) + 14

results_ch_cases <- get_summaries(sims_ch_cases, ch_max_date, params)
...
results_ch_cases$deaths <- length(ch_first_death_dates)
...
sp_first_death_dates <- rep(as.Date('2020-03-04'),2) ## There were two but not in data

sims_sp_cases <- sim_deaths(sp_first_death_dates, params)

sp_max_date <- max(sp_first_death_dates)

results_sp_cases <- get_summaries(sims_sp_cases, sp_max_date, params)
...
results_sp_cases$deaths <- length(sp_first_death_dates)
...
results_it_cases$country <- "Italy"
