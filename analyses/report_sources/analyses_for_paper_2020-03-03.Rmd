...
rfh_load_scripts()
path_to_simulator <- here("analyses", 
                          "rds", "simulate_cases.rds")
...
cases_path <- here::here("analyses",
                         "data",
           "bno_cases_data.csv")

df <- read.csv(cases_path, stringsAsFactors = FALSE) %>% 
  mutate(date_hm = as.Date(date, format = "%d/%m/%Y"))
...
france_deaths <- df %>% filter(country == "France",
                        new_death == 1)

france %>% filter(new_death == 1)

france_num_deaths <- sum(france_deaths$new_death)

france_death_first <- min(france_deaths$date)
france_death_latest <- max(france_deaths$date)
